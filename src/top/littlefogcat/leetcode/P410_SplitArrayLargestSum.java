...
public class P410_SplitArrayLargestSum {
    ...
    public int splitArray(int[] nums, int m) {
        average = (sum(nums, 0, nums.length - 1) + 0.0) / m;
        return splitArray(nums, 0, m);
    }

    int[][] startMArray = new int[1001][51];

    private int splitArray(int[] nums, int start, int m) {
        if (startMArray[start][m] != 0) return startMArray[start][m];
        if (m == 1) return sum(nums, start, nums.length - 1);
        if (start >= nums.length) return 0;
        int min = -1;
        int sum = 0;
        for (int end = start; end <= nums.length - m; end++) {
            sum += nums[end];
            if (sum < average && end < nums.length - m && sum + nums[end + 1] < average) {
                continue;
            }
            if (min != -1 && sum > min) break;
            int r = Math.max(sum, splitArray(nums, end + 1, m - 1));
            if (min == -1 || r < min) min = r;
        }
        startMArray[start][m] = min;
        return min;
    }
    ...
    public int splitArray1(int[] nums, int m) {
        if (m == 1) return sum(nums, 0, nums.length - 1);
        // 二分法
        int high = 0;
        int low = nums[0];
        for (int num : nums) {
